...
import { Calendar, Clock, CheckCircle, XCircle, Plus, Play, Check, X, Phone } from "lucide-react";
...
export default function AppointmentsPage() {
  const { appointments, updateAppointment, addAppointment, deleteAppointment } = useData();
  ...
  const handleStatusChange = (id: string, status: Appointment["status"]) => {
    ...
    toast({ title: statusMessages[status], description: "Status updated successfully." });
  };

  const handleSave = (data: { patientName: string; patientPhone: string; time: string; date: string; type: string; status: Appointment["status"]; notes?: string }) => {
